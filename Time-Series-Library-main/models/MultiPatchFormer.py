...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.out_linear_3 = torch.nn.Linear(
            self.d_model + 2 * self.pred_len // 8 -1, self.pred_len // 8
        )
        self.out_linear_4 = torch.nn.Linear(
            self.d_model + 3 * self.pred_len // 8 -1, self.pred_len // 8
        )
        self.out_linear_5 = torch.nn.Linear(
            self.d_model + self.pred_len // 2 -2, self.pred_len // 8
        )
        self.out_linear_6 = torch.nn.Linear(
            self.d_model + 5 * self.pred_len // 8-2, self.pred_len // 8
        )
        self.out_linear_7 = torch.nn.Linear(
            self.d_model + 6 * self.pred_len // 8-3, self.pred_len // 8
        )
        self.out_linear_8 = torch.nn.Linear(
            self.d_model + 7 * self.pred_len // 8-3,
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        def pad_tensor(tensor, target_len):
            ...
            return torch.nn.functional.pad(tensor, (0, 0, 0, pad_size))  # Pad on dim=1

        encoding_patch1 = pad_tensor(encoding_patch1, target_seq_len)
        encoding_patch2 = pad_tensor(encoding_patch2, target_seq_len)
        encoding_patch3 = pad_tensor(encoding_patch3, target_seq_len)
        encoding_patch4 = pad_tensor(encoding_patch4, target_seq_len)

        # print("encoding_patch1 shape:", encoding_patch1.shape)
        # print("encoding_patch2 shape:", encoding_patch2.shape)
        # print("encoding_patch3 shape:", encoding_patch3.shape)
        # print("encoding_patch4 shape:", encoding_patch4.shape)
        ...
        # print("encoding_1_ch shape:", encoding_1_ch.shape)
        # print("forecast_ch1 shape:", forecast_ch1.shape)
        # print("forecast_ch2 shape:", forecast_ch2.shape)
        # extra_dim = torch.zeros(encoding_1_ch.shape[0], encoding_1_ch.shape[1], 1, device=encoding_1_ch.device)
        # concatenated = torch.cat((encoding_1_ch, forecast_ch1, forecast_ch2, extra_dim), dim=-1)
        # forecast_ch3 = self.out_linear_3(concatenated)
